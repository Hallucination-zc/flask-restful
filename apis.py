...
class UserApi(Resource):  # 声明User资源
    ...
    def delete(self):
        ...
        # user = queryById(User, id)  # 是否id考虑不存在的情况
        # delete(user)
...
class MusicApi(Resource):
    parser = reqparse.RequestParser()  # 创建请求参数的解析器
    # 向参数解析器中添加请求参数
    ...
    def get(self):
        ...
        # 如果请求参数不满足条件，则直接返回参数相关的help说明
        args = self.parser.parse_args()
        # 从args中读取name请求参数的值
        name = args.get('name')
        tags = args.get('tag')
        # 从args中读取session(cookie中的session)
        session = args.get('session')
        print('session-->', session)
        musics = query(Music).filter(Music.name.like('%{}%'.format(name)))
        if musics.count():
            return {'data': musics.all()}
        return {'msg': '歌曲:{}---不存在,标签：{}！'.format(name, tags)}
